...
trait XML{
     ...
     * @return array|bool
     */
    static public function toArray($xml, $config = [], &$errorMessage = NULL){
        try{
            $result = XML2Array::createArray($xml, $config);
            if( ! empty($config['filterNameSpace']) && $config['filterNameSpace'] === TRUE){
                $result = self::filterNameSpace($result);
            }
        }catch(\Exception $e) {
    ...
    static public function filterNameSpace($source){
        $newKey = function($rawKey){
            $key = $rawKey;
            $index = strpos($rawKey, ':', 1);
            if($index !== FALSE){
                $key = substr($rawKey, $index + 1);
            }
            return $key;
        };
        return self::replaceArrayKeyBy($source, $newKey);
    }

    static public function replaceArrayKeyBy($source, Callable $newKey = NULL){
        $result = [];
        if( ! empty($source) && is_array($source)){
            self::_replaceArrayKey($source, $result, $newKey);
        }
        return $result;
    }

    static protected function _replaceArrayKey($source, &$dist, Callable $newKey = NULL){
        if( ! empty($source)){
            foreach($source as $key => $value){
                if(is_array($value)){
                    $group = [];
                    self::_replaceArrayKey($value, $group, $newKey);
                }else{
                    $group = $value;
                }
                $key = $newKey($key);
